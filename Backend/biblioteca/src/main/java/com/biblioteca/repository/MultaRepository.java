...
public interface MultaRepository extends JpaRepository<Multa, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT m.prestamo.usuario.id, m.prestamo.usuario.nombre, SUM(m.total) as deuda FROM Multa m WHERE m.pagada = false GROUP BY m.prestamo.usuario.id, m.prestamo.usuario.nombre ORDER BY deuda DESC")
